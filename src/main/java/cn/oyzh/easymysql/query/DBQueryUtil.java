...
public class DBQueryUtil {
     ...
     * @return 结果
     */
    public static List<DBQueryPromptItem> initPrompts(DBQueryToken token, float minCorr) {
        ...
        if (token.isPossibilityKeyword()) {
            tasks.add(() -> DBQueryUtil.getKeywords().parallelStream().forEach(keyword -> {
                // 计算相关度
                double corr = DBQueryUtil.clacCorr(keyword, text);
                if (corr > minCorr) {
                    DBQueryPromptItem item = new DBQueryPromptItem();
                    item.setType((byte) 4);
        ...
        if (token.isPossibilityDatabase()) {
            tasks.add(() -> DBQueryUtil.getDatabases().parallelStream().forEach(database -> {
                // 计算相关度
                double corr = DBQueryUtil.clacCorr(database.getName(), text);
                if (corr > minCorr) {
                    DBQueryPromptItem item = new DBQueryPromptItem();
                    item.setType((byte) 1);
        ...
        if (token.isPossibilityTable()) {
            tasks.add(() -> DBQueryUtil.getTables().parallelStream().forEach(dbTable -> {
                // 计算相关度
                double corr = DBQueryUtil.clacCorr(dbTable.getName(), text);
                if (corr > minCorr) {
                    DBQueryPromptItem item = new DBQueryPromptItem();
                    item.setType((byte) 2);
                    item.setCorrelation(corr);
        ...
        if (token.isPossibilityView()) {
            tasks.add(() -> DBQueryUtil.getViews().parallelStream().forEach(dbTable -> {
                // 计算相关度
                double corr = DBQueryUtil.clacCorr(dbTable.getName(), text);
                if (corr > minCorr) {
                    DBQueryPromptItem item = new DBQueryPromptItem();
        ...
        if (token.isPossibilityFunction()) {
            tasks.add(() -> DBQueryUtil.getFunctions().parallelStream().forEach(function -> {
                // 计算相关度
                double corr = DBQueryUtil.clacCorr(function.getName(), text);
                if (corr > minCorr) {
                    DBQueryPromptItem item = new DBQueryPromptItem();
                    item.setType((byte) 6);
                    item.setCorrelation(corr);
        ...
        if (token.isPossibilityProcedure()) {
            tasks.add(() -> DBQueryUtil.getProcedures().parallelStream().forEach(procedure -> {
                // 计算相关度
                double corr = DBQueryUtil.clacCorr(procedure.getName(), text);
                if (corr > minCorr) {
                    DBQueryPromptItem item = new DBQueryPromptItem();
                    item.setType((byte) 7);
                    item.setCorrelation(corr);
        ...
        if (token.isPossibilityColumn()) {
            tasks.add(() -> DBQueryUtil.getColumns().parallelStream().forEach(column -> {
                // 计算相关度
                double corr = DBQueryUtil.clacCorr(column.getName(), text);
                if (corr > minCorr) {
                    DBQueryPromptItem item = new DBQueryPromptItem();
                    item.setType((byte) 3);
                    item.setCorrelation(corr);
        ...
        // 根据相关度排序
        List<DBQueryPromptItem> itemList = items.parallelStream().sorted(Comparator.comparingDouble(DBQueryPromptItem::getCorrelation)).collect(Collectors.toList());
        // 反转列表
